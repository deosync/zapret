...
# Rule 5: TCP 80,443
config="$config --filter-tcp=80,443 --hostlist="$MODPATH/list/list-general.txt" --hostlist-exclude="$MODPATH/list/list-exclude.txt" --ipset-exclude="$MODPATH/ipset/ipset-exclude.txt" --dpi-desync=multisplit --dpi-desync-split-seqovl=652 --dpi-desync-split-pos=2 --dpi-desync-split-seqovl-pattern="$MODPATH/fake/tls_clienthello_www_google_com.bin" --new"

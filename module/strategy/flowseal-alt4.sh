#!/bin/bash
# Zapret Configuration - general (ALT12)
...
config="$config --filter-udp=19294-19344,50000-50100 --filter-l7=discord,stun --dpi-desync=fake --dpi-desync-fake-discord=$MODPATH/fake/stun.bin --dpi-desync-fake-discord=$MODPATH/fake/quic_initial_dbankcloud_ru.bin --dpi-desync-fake-stun=$MODPATH/fake/quic_initial_dbankcloud_ru.bin --dpi-desync-repeats=3 --new"
...
config="$config --filter-tcp=443 --hostlist=$MODPATH/list/list-google.txt --ip-id=zero --dpi-desync=hostfakesplit --dpi-desync-fooling=ts --dpi-desync-hostfakesplit-mod=host=www.google.com --new"

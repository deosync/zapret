...
config="$config --filter-tcp=80,443 --hostlist=%LISTS%list-general.txt --hostlist-exclude=%LISTS%list-exclude.txt --ipset-exclude=%LISTS%ipset-exclude.txt --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fooling=ts --dpi-desync-fake-tls=$MODPATH/fake/tls_clienthello_www_google_com.bin --new"
...
config="$config --filter-tcp=80,443 --ipset=%LISTS%ipset-all.txt --hostlist-exclude=%LISTS%list-exclude.txt --ipset-exclude=%LISTS%ipset-exclude.txt --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fooling=ts --dpi-desync-fake-tls=$MODPATH/fake/tls_clienthello_www_google_com.bin --new"

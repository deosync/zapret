#!/bin/bash
# Zapret Configuration - general (SIMPLE FAKE ALT) (ALT)
...
config="$config --filter-tcp=443 --hostlist=$MODPATH/list/list-google.txt" --ip-id=zero --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fooling=badseq --dpi-desync-badseq-increment=10000000 --dpi-desync-fake-tls=$MODPATH/fake/tls_clienthello_www_google_com.bin" --new"

# Rule 5: TCP 80,443 для основного списка
config="$config --filter-tcp=80,443 --hostlist=$MODPATH/list/list-general.txt" --hostlist-exclude=$MODPATH/list/list-exclude.txt" --ipset-exclude=$MODPATH/list/ipset-exclude.txt" --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fooling=badseq --dpi-desync-badseq-increment=10000000 --dpi-desync-fake-tls=$MODPATH/fake/tls_clienthello_www_google_com.bin" --new"
...
config="$config --filter-tcp=80,443, --ipset=$MODPATH/list/ipset-all.txt" --hostlist-exclude=$MODPATH/list/list-exclude.txt" --ipset-exclude=$MODPATH/list/ipset-exclude.txt" --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-fooling=badseq --dpi-desync-badseq-increment=10000000 --dpi-desync-fake-tls=$MODPATH/fake/tls_clienthello_www_google_com.bin" --new"

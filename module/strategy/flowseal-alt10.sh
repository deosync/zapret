...
config="$config --filter-tcp=443 --hostlist=%LISTS%list-google.txt --ip-id=zero --dpi-desync=hostfakesplit --dpi-desync-repeats=4 --dpi-desync-fooling=ts --dpi-desync-hostfakesplit-mod=host=www.google.com --new"

# Rule 5: TCP 80,443 для основного списка
config="$config --filter-tcp=80,443 --hostlist=%LISTS%list-general.txt --hostlist-exclude=%LISTS%list-exclude.txt --ipset-exclude=%LISTS%ipset-exclude.txt --dpi-desync=hostfakesplit --dpi-desync-repeats=4 --dpi-desync-fooling=ts,md5sig --dpi-desync-hostfakesplit-mod=host=ozon.ru --new"
...
config="$config --filter-tcp=80,443, --ipset=%LISTS%ipset-all.txt --hostlist-exclude=%LISTS%list-exclude.txt --ipset-exclude=%LISTS%ipset-exclude.txt --dpi-desync=hostfakesplit --dpi-desync-repeats=4 --dpi-desync-fooling=ts --dpi-desync-hostfakesplit-mod=host=ozon.ru --new"

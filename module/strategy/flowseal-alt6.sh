...
config="$config --filter-tcp=%GameFilterTCP% --ipset=$MODPATH/ipset/ipset-all.txt --ipset-exclude=$MODPATH/ipset/ipset-exclude.txt --ipset-exclude=$MODPATH/ipset/ipset-exclude-user.txt --dpi-desync=fake,multisplit --dpi-desync-repeats=6 --dpi-desync-any-protocol=1 --dpi-desync-cutoff=n3 --dpi-desync-fooling=badseq --dpi-desync-badseq-increment=1000 --dpi-desync-fake-tls=$MODPATH/fake/stun.bin --dpi-desync-fake-tls=$MODPATH/fake/tls_clienthello_www_google_com.bin --dpi-desync-fake-http=$MODPATH/fake/tls_clienthello_max_ru.bin --new"

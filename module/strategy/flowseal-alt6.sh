...
config="$config --filter-l3=ipv4 --filter-tcp=443,2053,2083,2087,2096,8443, --hostlist-exclude=%LISTS%list-exclude.txt --ipset-exclude=%LISTS%ipset-exclude.txt --dpi-desync=syndata,multidisorder --new"
...
config="$config --filter-udp= --ipset=%LISTS%ipset-all.txt --ipset-exclude=%LISTS%ipset-exclude.txt --dpi-desync=fake --dpi-desync-autottl=2 --dpi-desync-repeats=14 --dpi-desync-any-protocol=1 --dpi-desync-fake-unknown-udp=$MODPATH/fake/quic_initial_www_google_com.bin --dpi-desync-cutoff=n3 --new"

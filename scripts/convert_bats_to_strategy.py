...
def clean_line(line: str) -> str | None:
    """Удаляем служебные строки и символ ^ переноса"""
    s = line.strip()
    if not s or s.startswith(('::', '@echo', 'chcp', 'cd ', 'call ', 'set ', 'start ')):
        return None
    return s.rstrip('^').strip()

def normalize_paths(line: str) -> str:
    """Заменяем пути Windows на Unix-подобные и переменные"""
...
def fix_filters(rule: str, index: int) -> str:
    """Корректировка пустых фильтров и дубликатов"""
    rule = normalize_paths(rule)
    # Пустой tcp
    if '--filter-tcp=' in rule:
        rule = re.sub(r'--filter-tcp=,', '--filter-tcp=80,443', rule)
        rule = re.sub(r'--filter-tcp=$', '--filter-tcp=80,443', rule)
    # Пустой udp (для catch-all)
...
def parse_bat_file(bat_file: Path) -> list[str]:
    lines = [clean_line(l) for l in bat_file.read_text(encoding='utf-8', errors='ignore').splitlines()]
    ...
    with out_file.open('w', encoding='utf-8') as f:
        ...
        for i, part in enumerate(parts, start=1):
            ...
            comment_map = {
                ...
                8: 'UDP для ipset-all (catch-all)',
            }
            comment = f'# Rule {i}: {comment_map.get(i, "")}'
            if i == 1:
                f.write(f'{comment}\n')
